...
public interface RoundAnswerRepository extends CrudRepository<RoundAnswer, Integer> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT r FROM RoundAnswer r WHERE r.round_id.game_match.id = :matchId AND r.player_id = :playerId")
    java.util.List<RoundAnswer> findAllPlayerAnswersInMatch(@org.springframework.data.repository.query.Param("matchId") Integer matchId, @org.springframework.data.repository.query.Param("playerId") Integer playerId);
}
